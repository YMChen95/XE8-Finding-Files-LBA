	...
	if(file_pos==1){
		...
		for (int current_filepos = 1; current_filepos < file_pos; current_filepos++) {
			...
			if(file_pos-1==current_filepos){
				...
				int Physical_Lba_offset = GetPhysicalLBA(file_pos, offset_temp,MFTZero_Lba,fileName,phy_to_logi);
...
int TForm1::GetPhysicalLBA(int filepos, int offset_temp, int MFTZero_Lba, char fileName[64][64], int phy_to_logi)
		...
		if(HextoDec(jumpI_range,4)==144){//90属性
			...
			if(HextoDec(jumpI_range,4)==160){//with A0属性
				...
				while (!find_file_flag) {
					...
					//找到索引项对应
					if (strcmp(fileNameIndx,fileName[filepos-1])==0) {
						memset(jumpI_range,0,sizeof(jumpI_range));
						jumpI_range=GetRange(buffer_temp,indx_temp_offset,4);
						find_file_flag=true;
						offset_temp = MFTZero_Lba + HextoDec(jumpI_range,4)*2;
